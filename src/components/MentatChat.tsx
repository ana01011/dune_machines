...
import { Send, Mic, Paperclip, Settings, Target, ArrowLeft, ChevronDown, Trash2, CreditCard as Edit2, Plus, MessageSquare, Menu, X, Copy, Check, Sun, Moon, Palette, Users, RotateCcw, User } from 'lucide-react';
...
interface MentatChatProps {
  onBack: () => void;
  onNavigateToWorkflows?: () => void;
}

export const MentatChat: React.FC<MentatChatProps> = ({ onBack, onNavigateToWorkflows }) => {
  ...
  const [selectedVersion, setSelectedVersion] = useState('MENTAT');
  ...
  const availableThemes = getAvailableThemes('mentat');
              ...
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center py-4 sm:py-8">
                  ...
                  <p className="text-xs sm:text-sm text-white/50 font-light tracking-wide max-w-xs sm:max-w-md px-4">
                    The Human Computer awaits your queries
                  </p>
                </div>
              )}
