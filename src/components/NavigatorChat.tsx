...
import { Send, Mic, Paperclip, Settings, Globe, ArrowLeft, ChevronDown, Trash2, CreditCard as Edit2, Plus, MessageSquare, Menu, X, Copy, Check, Sun, Moon, Palette, Users, RotateCcw, User } from 'lucide-react';
...
export const NavigatorChat: React.FC<NavigatorChatProps> = ({ onBack, onNavigateToWorkflows }) => {
  ...
  const [selectedVersion, setSelectedVersion] = useState('NAVIGATOR');
  ...
  const availableThemes = getAvailableThemes('navigator');
              ...
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center py-4 sm:py-8">
                  ...
                  <p className="text-xs sm:text-sm text-white/50 font-light tracking-wide max-w-xs sm:max-w-md px-4">
                    The Path Finder awaits your queries
                  </p>
                </div>
              )}
